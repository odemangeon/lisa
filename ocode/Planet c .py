...
#    print(" The problem lies somewhere here ", (yfunc-y_model)**2*inv_sigma)
#    print( " /n and maybe here too.. ",  - np.log(inv_sigma))
    return -0.5*(np.sum((yfunc-y_model)**2*inv_sigma - np.log(inv_sigma)))
    
def lnpriornew(theta_free):
    k, t00, t1, t2, t3, a, i, e, ffunc = theta_free
    if  0.01 < ffunc < 1.1 and 0 < k < 0.1 and 10 < a < 40 and 1 < i < np.pi and 0 <= e < 1 and 56817.156136 < t00 < 56817.456136 and 56840.9029457 < t1 < 56841.2598587 and 56864.7076936 < t2 < 56865.1037389 and 56888.5756528 < t3 < 56888.9008336 :
        return 0.0            
    return -np.inf   
...
#    lenwalksamp[y] = np.int(len(temp)) ##to keep track of total number of all useful walkers 
#%%
...
#np.percentile(samples[:,2], [16,50,84])

k_same = np.percentile(newcornersamples[:,0], [50])
t00_less,t00_same, t00_more = np.percentile(newcornersamples[:,1], [16,50,84])
t1_less,t1_same,t1_more = np.percentile(newcornersamples[:,2], [16,50,84])
t2_less,t2_same,t2_more = np.percentile(newcornersamples[:,3], [16,50,84])
t3_less,t3_same,t3_more = np.percentile(newcornersamples[:,4], [16,50,84])
a_same = np.percentile(newcornersamples[:,5], [50])
i_same = np.percentile(newcornersamples[:,6], [50])
e_same = np.percentile(newcornersamples[:,7], [50])
f_same = np.percentile(newcornersamples[:,8], [50])

#%%
#################### To get the correct corner plot, we need the correct values ######################
####################### Hence we need to remove the values which are zero ############################

fig = corner.corner(newcornersamples, labels=["$k$","$t00$","$t1$","$t2$","$t3$","$a$", "$i$", "$e$","$f$"])
