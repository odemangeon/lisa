...
##################
nwalker_fact = (
    4  # The number of walkers will be equal to nwalker_fact * nb of free parameters
)
...
check_convergence_every = 1000  # If different from None and > 0, emcee will check the autocorrelation step scale of the chain every 'check_convergence_every' steps
...
if "sinkid_file_explore" not in globals():
    sinkid_file_explore = logger.add(
        join(output_folders["log"], "exploration.log"), level="DEBUG"
    )
...
with open(
    join(output_folders["pickles_explore"], f"{obj_name}_param_names.json"), "w"
) as file:
