...
pl_kwargs = {f'RV_{obj_name}_SOPHIEp_0': {'fmt': 'o', 'color': 'C1', 'mfc': 'white', 'ms': 4, 'mew': 1, "alpha": 1., 'label': "SOPHIE+"},
...
pl_period = [df_fittedval.loc[f"{obj_name}_b_P"]["value"], df_fittedval.loc[f"{obj_name}_c_P"]["value"], df_fittedval.loc[f"{obj_name}_d_P"]["value"], df_fittedval.loc[f"{obj_name}_e_P"]["value"]]  # , df_fittedval.loc[f"{obj_name}_f_P"]["value"]
...
for dst in l_RV_datasets:
    ...
    pl_kwargs_jitter[dst]["alpha"] = pl_kwargs_jitter[dst].pop("alpha_jitter") if "alpha_jitter" in pl_kwargs_jitter[dst] else pl_kwargs_jitter[dst]["alpha"] / 2
...
ax_gls[-1].set_xlabel("Frequency [Hz]", fontsize=fontsize)
# create and set the twiny axis
ax_gls_twin = []
for ii in range(nb_axes):
...
for ii, key in enumerate(l_gls_key):
    ...
    # Plot the GLS in period
    ax_gls_twin[ii].plot(1 / gls.freq, gls.power, 'k-', label=gls_inputs[key]["label"], color="w", lw=0.5)
    xlim_per = ax_gls_twin[ii].get_xlim()
    ax_gls_twin[ii].set_xlim(xlim_per[::-1])  # To reverse the axis and correspond to the frequency orientation
    # Plot the GLS in frequency
    ax_gls[ii].plot(gls.freq / (24 * 60 * 60), gls.power, 'k-', label=gls_inputs[key]["label"], linewidth=gls_inputs[key]["linewidth"])
    ...
    ylims = ax_gls_twin[ii].get_ylim()
    xlims = ax_gls_twin[ii].get_xlim()
    for per, lab, align, xshift, yshift, color in zip(act_period, act_label, act_align, act_xshift, act_yshift, act_color):
        ax_gls_twin[ii].vlines(per, *ylims, colors="k", linestyles="dotted", linewidth=0.5, color=color)
        if key == "data":
            ax_gls_twin[ii].text(per + xshift, ylims[0] + yshift * (ylims[1] - ylims[0]), lab, horizontalalignment=align, color=color, fontsize=fontsize)
    for per, lab, align, shift, color in zip(pl_period, pl_label, pl_align, pl_shift, pl_color):
        ax_gls_twin[ii].vlines(per, *ylims, colors="k", linestyles="dashed", linewidth=0.5, color=color)
        if key == "data":
            ax_gls_twin[ii].text(per + shift, ylims[0] + 0.9 * (ylims[1] - ylims[0]), lab, horizontalalignment=align, color=color, fontsize=fontsize)
    ...
    for fap_ii, ls_ii, align, yshift in zip(fap, fap_ls, fap_align, fap_yshift):
        ...
        if pow_ii < ylims[1]:
            ax_gls_twin[ii].hlines(pow_ii, *xlims, colors="k", linestyles=ls_ii, linewidth=0.5)
            ax_gls_twin[ii].text(xlims[0] + fap_xshift, pow_ii + yshift * (ylims[1] - ylims[0]),
