...
###############################
obj_name = "WASP-76"  # Change
kwargs_datasim = {}
star_kwargs = {"M": {"value": 1.487,  # TS3
                     "error": 0.060},
               "R": {"value": 1.863,  # TS3
                     "error": 0.033},
               "Teff": {"value": 6347,  # TS3
...
min_burnin = 2000  # Valeu of the burnin to use
...
if do_SecParam:
    ...
    occ_depth_chain = (chainI[f"{obj_name}_b_A"] / 2 + chainI[f"{obj_name}_b_Foffset"]) * 1e6

    l_param_chainIsec = l_param_name_sec + [f"{obj_name}_b_OccDepth", lnprobability_name]
    chainIsec = ChainsInterpret(np.dstack((chainIsec, occ_depth_chain, lnprobability)), l_param_chainIsec)
