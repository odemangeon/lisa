...
datasetnames = [f"LC_{obj_name}_CHEOPS_{ii}" for ii in range(3)] + [f"LC_{obj_name}_CHEOPS_{ii + 100}" for ii in range(20)]
row4datasetname = {f"LC_{obj_name}_CHEOPS_{ii}": 0 for ii in range(3)}
row4datasetname.update({f"LC_{obj_name}_CHEOPS_{ii + 100}": 0 for ii in range(20)})
pl_kwargs = {f"LC_{obj_name}_CHEOPS_{ii}": {"label": None} for ii in range(3)}
pl_kwargs.update({f"LC_{obj_name}_CHEOPS_{100 + ii}": {"label": None} for ii in range(20)})
...
create_LC_phasefolded_plots(fig=fig, post_instance=post_instance, df_fittedval=df_fittedval, datasim_kwargs=kwargs_datasim,
                            ...
                            pl_kwargs=pl_kwargs,
                            xlims=None, force_xlims=False,
                            ylims={"data": {"all": (-250, 500)}, "resi": {"all": (-375, 375)}},
                            rms_kwargs=None,
                            legend_kwargs=None,
