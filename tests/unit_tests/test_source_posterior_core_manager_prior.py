...
class TestMethods(unittest.TestCase):
    ...
    def test_manage_priors_database(self):
        ...
        list_res = manager.get_available_priors()
        logger.info("Available priors types: {}".format(list_res))
        self.assertSequenceEqual(list_res, [pf.UniformPrior.prior_type, ])
        self.assertTrue(manager.is_available_priortype(prior_type=pf.UniformPrior.prior_type))
        prior_subclass = manager.get_priorfunc_subclass(prior_type=pf.UniformPrior.prior_type)
        self.assertEqual(prior_subclass, pf.UniformPrior)
        self.assertFalse(manager.is_available_priortype(prior_type="Truc"))
        manager._reset_priors_database()
        list_res = manager.get_available_priors()
