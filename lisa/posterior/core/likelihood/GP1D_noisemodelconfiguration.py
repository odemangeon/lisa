...
try:
    ...
    from george.kernels import CosineKernel, ExpSine2Kernel, ExpSquaredKernel
...
try:
    ...
    from celerite2.terms import Matern32Term, RotationTerm, SHOTerm, Term
...
class Core_GP1DModel(Core_1ModelConfig):
    ...
    def __init__(self, model_name, model_instance, dico_config_model=None):
        ...
        self._object_categories = {
            "GP": GP1D_paramcontainer
        }  # This are parameter containers required by the model that host the model parameters
        self.load_config(dico_config=dico_config_model)

    def create_parameters_and_set_main(self, object_category=None):
        super().create_parameters_and_set_main(object_category=object_category)
    ...
    def _set_parametrisation(self, parametrisation=None):
        """ """
        self.parametrisation.update(
            {
                "log10": {
                    param_basename: False for param_basename in self._get_l_parameter_basename_GP()
                }
            }
        )
        if parametrisation is not None:
            for key in parametrisation:
                if key not in ["log10"]:
                    raise ValueError(
                        f"The only possible keys to the 'parametrisation' are 'log10'. You provided {key}."
                    )
                if key == "log10":
                    if not (isinstance(parametrisation[key], dict)):
                        raise ValueError(
                            f"'log10' in parametrisation should be a dictionary. You provided a {type(parametrisation[key])}."
                        )
                    for param_basename in parametrisation[key]:
                        if param_basename not in self._get_l_parameter_basename_GP():
                            raise ValueError(
                                f"The only possible keys to the 'log10' dictionary are {self._get_l_parameter_basename_GP()}. You provided {key}."
                            )
                    if not (isinstance(parametrisation[key][param_basename], bool)):
                        raise ValueError(
                            f"Value of parametrisation[{key}][{param_basename}] should be a bool (got {parametrisation[key][param_basename]})"
                        )
                    self.parametrisation[key][param_basename] = parametrisation[key][param_basename]
        super()._set_parametrisation(parametrisation=parametrisation)
    ...
    def _get_function_get_l_parameter_basename(self, object_category):
        ...
        super()._get_function_get_l_parameter_basename(object_category=object_category)
    ...
    def _get_function_get_kwargs_4_get_l_parameter_basename(self, object_category):
        ...
        super()._get_function_get_kwargs_4_get_l_parameter_basename(object_category=object_category)
    ...
    def _get_function_get_parameter_name(self, object_category):
        ...
        super()._get_function_get_parameter_name(object_category=object_category)
    ...
    def _get_function_get_kwargs_4_get_parameter_name(self, object_category):
        ...
        super()._get_function_get_kwargs_4_get_parameter_name(object_category=object_category)
    ...
    def _get_function_create_parameter(self, object_category):
        ...
        super()._get_function_create_parameter(object_category=object_category)

    def _get_function_get_kwargs_4_create_parameter(self, object_category):
        if object_category == "GP":
            return self._get_kwargs_4_create_parameter_default
        super()._get_function_get_kwargs_4_create_parameter(object_category=object_category)
    ...
    def _get_function_get_parameter(self, object_category):
        ...
        super()._get_function_get_parameter(object_category=object_category)

    def _get_function_get_kwargs_4_get_parameter(self, object_category):
        if object_category == "GP":
            return self._get_kwargs_4_get_parameter_default
        super()._get_function_get_kwargs_4_get_parameter(object_category=object_category)
    ...
    def _add_GP_parameter_and_get_parameter_text(self, function_builder, l_function_shortname):
        ...
        for param_basename, param in dico_param["GP"].items():
            for func_shortname in l_function_shortname:
                function_builder.add_parameter(
                    parameter=param, function_shortname=func_shortname, exist_ok=True
                )
                dico[func_shortname][param_basename] = function_builder.get_text_4_parameter(
                    parameter=param, function_shortname=func_shortname
                )
                if self.log10(param_basename=param_basename):
                    dico[func_shortname][param_basename] = f"10**{dico[param_basename]}"
                    function_builder.add_variable_to_ldict(
                        variable_name="log",
                        variable_content=log,
    ...
    def add_text_lnlike(
        ...
        l_function_shortname_add_param_only=None,
    ):
        ...
        dico_text_param = self._add_GP_parameter_and_get_parameter_text(
            function_builder=function_builder,
            l_function_shortname=l_function_shortname + l_function_shortname_add_param_only,
        )
        self._add_text_GP_kernel(
            dico_text_param=dico_text_param,
            function_builder=function_builder,
            l_function_shortname=l_function_shortname,
        )
        self._add_text_compute_lnlike(
            text_sorted_kwargs=text_sorted_kwargs,
            function_builder=function_builder,
            l_function_shortname=l_function_shortname,
        )
    ...
    def _add_text_compute_lnlike(self, function_builder, l_function_shortname):
        ...
        raise NotImplementedError(
            f"You need to implement _add_text_compute_lnlike in class {self.__class__}"
        )
    ...
    def _add_text_compute_gpsim(self, text_sorted_kwargs, function_builder, l_function_shortname):
        """Add the text of the GP kernel and the text to return the GP simulation/prediction"""
        raise NotImplementedError(
            f"You need to implement _add_text_compute_gpsim in class {self.__class__}"
        )
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        """Add the text of the GP kernel and the text to return the GP simulation/prediction"""
        raise NotImplementedError(
            "You need to implement _add_text_GP_kernel in class {self.__class__}"
        )
...
class QPGeorgeModel(Core_GP1DModel_George):
        ...
        for param_basename, param in dico_param["GP"].items():
            for func_shortname in l_function_shortname:
                ...
                if param_basename in ["gamma"]:
                    if self.log10(param_basename=param_basename):
                        dico[func_shortname][param_basename] = (
                            f"10**{dico[func_shortname][param_basename]}"
                        )
                if param_basename == "P":
                    if self.log10(param_basename=param_basename):
                        dico[func_shortname][param_basename] = (
                            f"{dico[func_shortname][param_basename]} * log(10)"
                        )
                    else:
                        dico[func_shortname][param_basename] = (
                            f"log({dico[func_shortname][param_basename]})"
                        )
                    function_builder.add_variable_to_ldict(
                        variable_name="log",
                        variable_content=log,
                ...
                if param_basename in ["tau", "A"]:
                    if self.log10(param_basename=param_basename):
                        dico[func_shortname][param_basename] = (
                            f"2 * {dico[func_shortname][param_basename]} * log(10)"
                        )
                    else:
                        dico[func_shortname][param_basename] = (
                            f"2 * log({dico[func_shortname][param_basename]})"
                        )
                    function_builder.add_variable_to_ldict(
                        variable_name="log",
                        variable_content=log,
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            kernel = (
                1**2
                * ExpSquaredKernel(metric=20**2)
                * ExpSine2Kernel(gamma=1 / (2 * 0.5**2), log_period=log(10))
            )
            ...
            function_builder.add_to_body_text(
                text=text_GP_kernel, function_shortname=func_shortname
            )
            function_builder.add_variable_to_ldict(
                variable_name="ExpSquaredKernel",
                variable_content=ExpSquaredKernel,
                function_shortname=func_shortname,
                exist_ok=True,
                overwrite=False,
            )
            function_builder.add_variable_to_ldict(
                variable_name="ExpSine2Kernel",
                variable_content=ExpSine2Kernel,
                function_shortname=func_shortname,
                exist_ok=True,
                overwrite=False,
            )
...
class QPCGeorgeModel(Core_GP1DModel_George):
    ...
    ldict_kernel = {
        "ExpSquaredKernel": ExpSquaredKernel,
        "ExpSine2Kernel": ExpSine2Kernel,
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            kernel = (
                1**2
                * ExpSquaredKernel(metric=20)
                * (
                    ExpSine2Kernel(gamma=1 / (2 * 0.5**2), log_period=log(10))
                    + 0.1 * CosineKernel(log_period=log(10) - log(2))
                )
            )
            gp = GP(kernel, fit_kernel=True, mean=0, fit_mean=False)
            function_builder.add_variable_to_ldict(
            ...
            function_builder.add_variable_to_ldict(
                variable_name="CosineKernel",
                variable_content=CosineKernel,
                function_shortname=func_shortname,
                exist_ok=True,
                overwrite=False,
            )
...
class QPCeleriteModel(Core_GP1DModel_Celerite):
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            ...
            function_builder.add_variable_to_ldict(
                variable_name="CustomTerm",
                variable_content=self.CustomTerm,
                function_shortname=func_shortname,
                exist_ok=False,
                overwrite=False,
            )
...
class RotationCeleriteModel(Core_GP1DModel_Celerite):
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            ...
            function_builder.add_variable_to_ldict(
                variable_name="RotationTerm",
                variable_content=RotationTerm,
                function_shortname=func_shortname,
                exist_ok=False,
                overwrite=False,
            )
...
class SHOCeleriteModel(Core_GP1DModel_Celerite):
        ...
        if parametrisation is not None:
            for key in parametrisation:
                if key not in ["use_rho", "use_Q", "use_A", "log10"]:
                    raise ValueError(
                        f"The only possible keys to the 'parametrisation' are ['use_rho', 'use_Q', 'use_A', 'log10']. You provided {key}."
                    )
                if key in ["use_rho", "use_Q", "use_A"]:
                    if not (isinstance(parametrisation[key], bool)):
                        raise ValueError(
                            f"Value of parametrisation[{key}] should be a bool (got {parametrisation[key]})"
                        )
                ...
                if key == "log10":
                    ...
                    for param_basename in parametrisation[key]:
                        if param_basename not in ["rho/omega0", "Q/tau", "A/S0"]:
                            raise ValueError(
                                f"The only possible keys to the 'log10' dictionary are ['rho/omega0', 'Q/tau', 'A/S0']. You provided {key}."
                            )
                        if not (isinstance(parametrisation[key][param_basename], bool)):
                            raise ValueError(
                                f"Value of parametrisation[{key}][{param_basename}] should be a bool (got {parametrisation[key][param_basename]})"
                            )
                        self.parametrisation[key][param_basename] = parametrisation[key][
                            param_basename
                        ]

        Core_1ModelConfig._set_parametrisation(self, parametrisation=parametrisation)
    ...
    @property
    def use_A(self):
        return self.parametrisation["use_A"]

    @property
    def use_rho(self):
        return self.parametrisation["use_rho"]

    @property
    def use_Q(self):
        return self.parametrisation["use_Q"]
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            ...
            function_builder.add_variable_to_ldict(
                variable_name="SHOTerm",
                variable_content=SHOTerm,
                function_shortname=func_shortname,
                exist_ok=False,
                overwrite=False,
            )
...
class Matern32CeleriteModel(Core_GP1DModel_Celerite):
    ...
    def _add_text_GP_kernel(self, dico_text_param, function_builder, l_function_shortname):
        ...
        for func_shortname in l_function_shortname:
            ...
            function_builder.add_variable_to_ldict(
                variable_name="Matern32Term",
                variable_content=Matern32Term,
                function_shortname=func_shortname,
                exist_ok=False,
                overwrite=False,
            )
