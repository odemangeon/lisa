...
class GaussianModel(Core_1ModelConfig):
    ...
    def add_text_compute_lnlike(self, nparam_datasim, function_builder_1inst, function_shortname_1inst):
        ...
        if self.use_Baluevfactor:
            text_return = f"\n{tab}return -0.5 * npsum(((((concatenate(dict_datakwargs['data']) - concatenate(sim_data)).reshape((-1))) / concatenate(dict_datakwargs['data_err']))**2 / (1 - (nparam_datasim / len(dict_datakwargs['data_err'])))) + nplog(twopi * (concatenate(dict_datakwargs['data_err']))**2))"
        else:
            text_return = f"\n{tab}return -0.5 * npsum((((concatenate(dict_datakwargs['data']) - concatenate(sim_data)).reshape((-1))) / concatenate(dict_datakwargs['data_err']))**2 + nplog(twopi * (concatenate(dict_datakwargs['data_err']))**2))"
