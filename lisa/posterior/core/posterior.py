...
class Posterior(Named, RunFolderAttr, DstDbLockAttr, ConfigFileAttr):
    ...
    def compute_model(self, tsim, dataset_name, param, l_param_name, key_obj=None, datasim_kwargs=None,
        ...
        if supersamp > 1:
            ...
            # if Delta_tsim < exptime, t_model will not be an order vector anymore and that can cause problem down the line
            idx_sort_t_model = argsort(t_model)
            t_model = t_model[idx_sort_t_model]
        else:
            t_model = tsim
        ...
        if key_obj in self.datasimulators.instrument_db[instmod_fullname]:
            ...
            if noise_model.has_GP:
                ...
                model_allsameGPkernel = self.model.create_datasimulator_4_ldataset(l_dataset_obj=l_dataset_obj, dataset_db=self.dataset_db)[self.model.key_whole]
                ...
                gp_simulator, datasets_kwargs, l_gp_sim_param_full_name = noise_model._get_prefilledgpsim(l_instmod_obj=l_instmod_obj, l_dataset_obj=l_dataset_obj)
                ...
                if supersamp > 1:
                    ...
                    gp_pred_var = gp_pred_var[idx_desort_t_model]
