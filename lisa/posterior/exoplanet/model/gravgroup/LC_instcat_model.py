...
class LC_InstCat_Model(Core_InstCat_Model, SuperSampExpTimeAttr):
    ...
    def isdefined_LCparamfile(self):
        ...
        return self.isdefined_paramfile_instcat(inst_cat=self.__inst_cat__)
    ...
    def datasim_creator(self, inst_models, datasets=None):
    ...
    def create_instcat_paramfile(self, paramfile_path=None, answer_overwrite=None, answer_create=None):
        ...
        self.paramfile4instcat[self.__inst_cat__] = file_path
    ...
    def read_LC_param_file(self):
        ...
        if self.isdefined_LCparamfile:
            ...
            raise IOError("Impossible to read LC parameter file: {}".format(self.paramfile4instcat[self.__inst_cat__]))
    ...
    def load_LC_config(self, dico_config):
        ...
        for dict_name in l_dico_model_name:
            if dict_name not in dico_config:
                raise ValueError(f"The file {self.paramfile4instcat[self.__inst_cat__]} is missing the {dict_name} dictionary.")
            dico_model = dico_config[dict_name]
            if 'do' not in dico_model:
                raise ValueError(f"The file {self.paramfile4instcat[self.__inst_cat__]}: The dictionary {dict_name} is missing the 'do' key.")
            if dico_model['do']:
                if not(set(l_key_modeldicts) == set(dico_model.keys())):
                    raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: the keys of the {dict_name} dictionary have to be {l_key_modeldicts}.")
                if not(isinstance(dico_model[self._key_inst_variable_LC_models], bool)):
                    raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: {dict_name}[{self._key_inst_variable_LC_models}] has to be a boolean.")
                if not(isinstance(dico_model[self._key_instspecific_dict_LC_models], dict)):
                    raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: {dict_name}[{self._key_instspecific_dict_LC_models}] has to be a dict.")
        ...
        if dico_config["transit_model"]["do"]:
            ...
            if "model" not in transit_model[self._key_allinst_dict_LC_models]:
                raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: the transit_model[{self._key_allinst_dict_LC_models}] dictionary is missing the 'model' key.")
            if transit_model[self._key_allinst_dict_LC_models]['model'] not in self._transit_models:
                raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: {transit_model[self._key_allinst_dict_LC_models]['model']} is not an available transit model.")
        ...
        if dico_config["phasecurve_model"]["do"]:
            ...
            if not(set(l_key_mandatory) == set(phasecurve_model[self._key_allinst_dict_LC_models][0].keys())):
                raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: the keys of phasecurve_model[{self._key_allinst_dict_LC_models}][0] should be {l_key_mandatory}.")
            if phasecurve_model[self._key_allinst_dict_LC_models][0]['model'] not in self._phasecurve_models:
                raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: {phasecurve_model[{self._key_allinst_dict_LC_models}][0]['model']} is not an available phasecurve model.")
            if phasecurve_model[self._key_allinst_dict_LC_models][0]['model'] == "spiderman":
                ...
                if not(set(l_arg_mand_sp) == set(phasecurve_model[self._key_allinst_dict_LC_models][0]['args'].keys())):
                    raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: the keys of phasecurve_model[{self._key_allinst_dict_LC_models}][0]['args'] should be {l_arg_mand_sp}.")
                if not("brightness_model" in phasecurve_model[self._key_allinst_dict_LC_models][0]['args']['ModelParams_kwargs']):
                    raise ValueError(f"In file {self.paramfile4instcat[self.__inst_cat__]}: the keys of phasecurve_model[{self._key_allinst_dict_LC_models}][0]['args']['ModelParams_kwargs'] is missing the 'brightness_model' key")
            else:
