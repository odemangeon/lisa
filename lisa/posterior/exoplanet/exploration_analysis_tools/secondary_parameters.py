...
def get_secondary_xrdataset(infdata, sec_params, model):
    """Return xarray.Dataset instance with the computed chain of the secondary parameters.
    ...
    for sec_par_fullname, dico_sec_par in sec_params.items():
        if dico_sec_par['kwargs'] is not None:
            ...
            for arg_name, arg_value in kwargs.items():
                if arg_value == 'shape(nwalker,niter)':
                    kwargs[arg_name] = (infdata.posterior.coords["chain"].shape[0], infdata.posterior.coords["draw"].shape[0])
        else:
            kwargs = {}
        if dico_sec_par['l_param_name'] is not None:
            l_chains_param = []
            for param_name in dico_sec_par['l_param_name']:
                if param_name in infdata.posterior:
                    l_chains_param.append(infdata.posterior[param_name].values)
                elif param_name in model.get_list_paramnames(main=True, free=False, recursive=True,
                                                             no_duplicate=True, recursive_naming=True, include_prefix=True
                                                             ):
                    param = model.get_parameter(name=param_name, notexist_ok=False, return_error=False,
                    ...
                    l_chains_param.append(ones((infdata.posterior.coords["chain"].shape[0], infdata.posterior.coords["draw"].shape[0])) * param.value)
                elif param_name in sec_par_datavars:
                    l_chains_param.append(sec_par_datavars[param_name][1])
                else:
                    raise ValueError(f"Parameter {param_name} is not found anywhere")
        else:
            l_chains_param = []
        sec_par_datavars[sec_par_fullname] = (('chain', 'draw'), dico_sec_par['function'](*l_chains_param, **kwargs))

    return Dataset(data_vars=sec_par_datavars, coords=infdata.posterior.coords, attrs=infdata.posterior.attrs)
