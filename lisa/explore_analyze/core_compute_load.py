...
from numpy import ones_like, size, sqrt
...
from .core_plot import ComputedModels_Database, Expression, ModelBinning
...
def is_valid_model_available(key_model, datasetname, post_instance):
    ...
    if key_model == "model":
        ...
        return post_instance.model.instcat_models[
            inst_mod.instrument.category
        ].decorrelation_model_config[inst_mod_fullname]["do"]
...
def compute_model(
    ...
    """
    logger.info(
        f"Start compute_model for expression {expression.expression} and dataset {datasetname}"
    )
    ...
    (computedmodel, _, times_found) = computedmodels_db.find_computed_model(
        expression=expression.expression,
        datasetname=datasetname,
        binning=ModelBinning(exptime=exptime, supersampling=supersampling),
        times=times,
    ...
    if computedmodel is None:
        ...
        for component_i in expression.components:
            (computedmodel_component_i, _, _) = computedmodels_db.find_computed_model(
                expression=component_i,
                datasetname=datasetname,
                binning=ModelBinning(exptime=exptime, supersampling=supersampling),
                times=times,
                df_param_value=df_param_value,
            ...
            if computedmodel_component_i is None:
                ...
                if component_i == "data":
                    ...
                    model, model_err = compute_raw_models_func(
                        tsim=times,
                        key_model=component_i,
                        datasetname=datasetname,
                        post_instance=post_instance,
                        df_param_value=df_param_value,
                        datasim_kwargs=datasim_kwargs,
                        exptime=exptime,
                        supersamp=supersampling,
                ...
                if model is None:
                    logger.warning(
                        f"Component {component_i} could not be computed for dataset {datasetname} and is set to zero."
                    )
                    components[component_i] = {"values": 0.0, "errors": model_err}
                else:
                    computedmodels_db.store_computed_model(
                        expression=component_i,
                        datasetname=datasetname,
                        binning=ModelBinning(exptime=exptime, supersampling=supersampling),
                        df_param_value=df_param_value,
                ...
                components[component_i] = {
                    "values": computedmodel_component_i.values,
                    "errors": computedmodel_component_i.errors,
                }

        (computedmodel, _, _) = computedmodels_db.find_computed_model(
            expression=expression.expression,
            datasetname=datasetname,
            binning=ModelBinning(exptime=exptime, supersampling=supersampling),
            times=times,
        ...
        if computedmodel is None:
            d_globals = {
                component_i: components[component_i]["values"]
                for component_i in expression.components
            }
            ...
            d_globals = {
                component_i: components[component_i]["errors"]
                if components[component_i]["errors"] is not None
                else 0.0
                for component_i in expression.components
            }
            d_globals["sqrt"] = sqrt
    ...
    logger.debug(
        f"Done compute_model for expression {expression.expression} and dataset {datasetname}"
    )
...
def load_datasets_and_models(
    ...
    amplitude_fact,
    compute_raw_models_func,
    remove_add_model_components_func,
    ...
    for datasetname in datasetnames:
        ...
        dico_outputs["dico_kwargs"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_all_datasetkwargs()
        )
        dico_outputs["times"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_datasetkwarg("time")
        )
        dico_outputs["datas"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_datasetkwarg("data")
        )
        dico_outputs["rawdatas"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_datasetkwarg("data")
        )
        dico_outputs["data_errs"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_datasetkwarg("data_err")
        )
        ...
        dico_outputs["data_err_jitters"][datasetname] = copy(
            post_instance.dataset_db[datasetname].get_datasetkwarg("data_err")
        )
        ...
        if dico_outputs["has_jitters"][datasetname]:
            if inst_mod.jitter.free:
                dico_outputs["dico_jitters"][datasetname]["value"] = copy(
                    df_param_value.loc[inst_mod.jitter.full_name]["value"]
                )
            else:
            ...
            dico_outputs["data_err_jitters"][datasetname] = sqrt(
                compute_jitteredvar(
                    data_err=dico_outputs["data_err_jitters"][datasetname],
                    jitter=dico_outputs["dico_jitters"][datasetname]["value"],
                )
            )
            dico_outputs["data_err_worwojitters"][datasetname] = dico_outputs["data_err_jitters"][
                datasetname
            ].copy()
        else:
            dico_outputs["data_err_worwojitters"][datasetname] = dico_outputs["data_errs"][
                datasetname
            ].copy()
        ...
        kwargs_compute_model_4_key_model["data"] = {
            "remove_dict": None,
            "add_dict": None,
        }  # The order is important data has to be first then model, GP and residuals
        ...
        kwargs_compute_model_4_key_model["residual"] = {
            "remove_dict": {"model": True, "GP": True, "decorrelation_likelihood": True},
            "add_dict": {"data": True},
        }
        for key_model in kwargs_compute_model_4_key_model_user:
            if key_model not in kwargs_compute_model_4_key_model:
                kwargs_compute_model_4_key_model[key_model] = kwargs_compute_model_4_key_model_user[
                    key_model
                ]
            else:
                kwargs_compute_model_4_key_model[key_model].update(
                    kwargs_compute_model_4_key_model_user[key_model]
                )

        for key_model, kwargs in kwargs_compute_model_4_key_model.items():
            if (key_model in ["GP", "model_wGP"]) and not (compute_GP_model):
                continue
            (dico_outputs["models"][datasetname], _) = compute_and_plot_model(
                tsim=dico_outputs["times"][datasetname],
                key_model=key_model,
                datasetname=datasetname,
                post_instance=post_instance,
                df_param_value=df_param_value,
                ...
                remove_dict=kwargs.get("remove_dict", None),
                add_dict=kwargs.get("add_dict", None),
                ...
                fact_tsim_to_xsim=None,
                plot_unbinned=False,
                plot_binned=False,
                ax=None,
                pl_kwarg=None,
                key_pl_kwarg=None,
                models=dico_outputs["models"][datasetname],
                get_key_compute_model_func=get_key_compute_model_func,
                kwargs_get_key_compute_model=kwargs_get_key_compute_model,
            )
        ...
        if "data_err" in dico_outputs["models"][datasetname]:
            coeff_err = (
                dico_outputs["models"][datasetname]["data_err"]
                / dico_outputs["data_errs"][datasetname]
            )
...
def compute_raw_models(
    ...
    if key_model == "decorrelation_likelihood":
        if f"{datasetname}_decorr_like" in post_instance.datasimulators.dataset_db:
            datasim_docfunc_decorr_like = post_instance.datasimulators.dataset_db[
                f"{datasetname}_decorr_like"
            ]
            p_vect = df_param_value["value"][
                datasim_docfunc_decorr_like.param_model_names_list
            ].array
        ...
        kwargs_get_key_compute_model = (
            kwargs_get_key_compute_model if kwargs_get_key_compute_model is not None else {}
        )
        key_compute_model = get_key_compute_model_func(
            key_model=key_model, **kwargs_get_key_compute_model
        )
...
def compute_data_err_jittered(
    data_err: NDArray[float], post_instance: Posterior, datasetname: str, df_param_value: DataFrame
) -> NDArray[float] | None:
    inst_mod_fullname = post_instance.datasimulators.get_instmod_fullname(datasetname)
    inst_mod = post_instance.model.instruments[inst_mod_fullname]
    noise_model = post_instance.model.get_noise_model(inst_mod.noise_model_category)
...
def compute_data_and_resi_for_data2plots(
    data2plot: Data2Plot,
    post_instance: Posterior,
    datasim_kwargs: dict,
    ...
    data, data_err, _ = compute_model(
        post_instance=post_instance,
        df_param_value=data2plot.df_param_value,
        datasim_kwargs=datasim_kwargs,
        compute_raw_models_func=compute_raw_models_func,
        expression=data2plot.expression,
        times=times_dataset,
    ...
    data_err_jitter = compute_data_err_jittered(
        data_err=data_err,
        post_instance=post_instance,
        datasetname=data2plot.datasetname,
        df_param_value=data2plot.df_param_value,
    ...
    # Compute residuals
    logger.info(
        f"Compute residuals {'for ' + name_data2plot if name_data2plot is not None else ''}"
    )
    expression_resi = Expression(expression="data - model - GP - decorrelation_likelihood")
    residuals, _, _ = compute_model(
        post_instance=post_instance,
        df_param_value=data2plot.df_param_value,
        datasim_kwargs=datasim_kwargs,
        compute_raw_models_func=compute_raw_models_func,
        expression=expression_resi,
        times=times_dataset,
        datasetname=data2plot.datasetname,
        exptime=None,
        supersampling=None,
