...
def create_TS_plots(post_instance:Posterior, df_fittedval:DataFrame,
    ...
    if subplotspec is not None:
        gs = GridSpecFromSubplotSpec(nrows=plotdef_TS.nb_rows, ncols=plotdef_TS.nb_cols, subplot_spec=subplotspec, **create_axes_main_gridspec)
    else:
        gs = GridSpec(nrows=plotdef_TS.nb_rows, ncols=plotdef_TS.nb_cols, figure=fig, **create_axes_main_gridspec)

    # Make sure that legend_kwargs is well defined
    legend_kwargs = check_kwargs_by_column_and_row(kwargs_user=legend_kwargs, l_row_name=list(range(plotdef_TS.nb_rows)), l_col_name=list(range(plotdef_TS.nb_cols)),
                                                   kwargs_def={'do': False}, kwargs_init={0: {i_row: {'do': True} for i_row in range(plotdef_TS.nb_rows)}}
                                                   )
